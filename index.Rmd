...
```{R}
Project_df %>% ggplot(aes(x=Oscar, y=vote_average, color=Oscar)) + ylab("Average Rating") + ggtitle("Distribution of Movie Ratings") + geom_boxplot(size=1) + geom_jitter(alpha=.05) + theme(legend.position = "none")
```
...
```{R}
Project_df %>% ggplot(aes(x=vote_average, fill=Oscar)) + ggtitle("Shape of Movie Ratings") + geom_histogram(alpha=.5) + scale_y_continuous(lim=c(0,400), breaks = seq(0,400,50))
Project_df %>% ggplot(aes(x=popularity, fill=Oscar)) + ggtitle("Distribution & Shape of Movie Ratings") + geom_density(alpha=.3) + geom_rug() + scale_x_continuous(lim=c(0,150))
...
```{R}
Project_df %>% na.omit() %>% ggplot(aes(x=Year,y=popularity, color=Award)) + geom_point() + scale_y_continuous(lim=c(0,150)) + scale_x_discrete(breaks = seq(0,2020,10)) 
```

This graph demonstrates that the popularity of Oscar winners and nominees are increasing with time, but more so for Oscar winners.
